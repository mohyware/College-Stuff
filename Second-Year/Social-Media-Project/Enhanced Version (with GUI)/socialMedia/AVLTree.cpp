...
int AVLTree::hashCode(std::string str, int INTERNAL_NUMBER) {
...
void AVLTree::printNodeWithValue(int value, AVLNode* Troot) {
    if (!Troot)
        Troot = this->root;

    if (Troot->left)
        printNodeWithValue(value, Troot->left);

    // Check if the current node has the desired value
    if (Troot->hashValue == value) {
        std::cout << "CODE: " << Troot->hashValue << '\n';
        std::cout << "#Activities : " << std::endl;
        std::cout << "***********" << std::endl;
        auto print_Activity = [&]()-> void {
    ...
    if (Troot->right)
        printNodeWithValue(value, Troot->right);
}
...
void AVLTree::print_Statistics(AVLNode* Troot) {
    ...
    std::cout << "User Name : " << Troot->user->getUserName() << "\t\t Code : " << Troot->hashValue << std::endl;
